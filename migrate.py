...
DB_CONFIG = {
    'host': '10.248.48.4', # Set IP here
    'port': 3306,
    'user': 'root', # Set your user here
    'password': 'secure-password',  # Set your password here
    'db': 'test1', # Set your database here
    'charset': 'utf8mb4', # Remove this if you're not sure
    'cursorclass': DictCursor  # Returns results as dictionaries
}

# Create context managers for database connections
@contextmanager
...
def get_hw_type(racktables_object_id):
    ...
    with get_db_connection() as connection:
        with get_cursor(connection) as cursor:
            ...
            return hw_types[uint["uint_value"]] if uint else None
...
def createObjectsInRackFromAtoms(atoms, rack_name, rack_id):
    ...
    for Id in atoms_dict:
        ...
        current_atom = atoms_dict[Id][0]["atom"]
        current_height = atoms_dict[Id][0]["unit_no"]
        ...
        for atom in atoms_dict[Id].copy():
            ...
            if debug_splits:
                print(atom["unit_no"], current_height)

            # Look for device on a height above the last device
            # Once found a split based on the last 
            if atom["unit_no"] > current_height + 1 and current_counter > 0: # or (internal_separated_Ids == True and current_counter == max_counter):
    ...
    for Id in atoms_dict:
        ...
        manufacturer, device_role, device_type_model = get_manufacturer_role_type(real_id, objtype_id, height, is_full_depth)
...
def get_custom_fields(racktables_object_id, initial_dict=None):
...
with get_db_connection() as connection:
    with get_cursor(connection) as cursor:
        ...
        for row in cursor.fetchall():
            hw_types[row["dict_key"]] = row["dict_value"].strip("[]").split("|")[0].strip().replace("%"," ")
        ...
        for row in yellow_attributes:
            Id, Type, name = row["id"], row["type"], row["name"]
...
#   print("\n\nPaste that in the intializers/custom_fields.yml file for this program to work!")
    ...
    with get_db_connection() as connection:
        with get_cursor(connection) as cursor:
            ...
            create_global_tags(row["tag"] for row in cursor.fetchall())
    ...
    if CREATE_VLAN_GROUPS:
        ...
        with get_db_connection() as connection:
            with get_cursor(connection) as cursor:
                ...
                for row in vlans_domains:
                    Id, description = row["id"], row["description"]
            ...
            with get_db_connection() as connection:
                with get_cursor(connection) as cursor:
                    ...
                    for row in vlans:
                        ...
                        cursor.execute("SELECT vlan_descr FROM VLANDescription WHERE domain_id=%s AND vlan_id=%s", 
                                     (domain_id, vlan_id))
                        result = cursor.fetchone()
    ...
    if CREATE_MOUNTED_VMS:
        ...
        with get_db_connection() as connection:
            with get_cursor(connection) as cursor:
                ...
                for row in clusters:
                    Id, cluster_name, asset_no, label = row["id"], row["name"], row["asset_no"], row["label"]
                    ...
                    cursor.execute("SELECT child_entity_type,child_entity_id FROM EntityLink WHERE parent_entity_id=%s", (Id,))
                    ...
                    for child_row in child_virtual_machines:
                        ...
                        cursor.execute("SELECT name,label,comment,objtype_id,asset_no FROM Object WHERE id=%s", 
                                     (child_entity_id,))
                        vm_row = cursor.fetchone()
                        if vm_row:
                            virtual_machine_name = vm_row["name"]
                            virtual_machine_label = vm_row["label"]
                            virtual_machine_comment = vm_row["comment"]
                            virtual_machine_objtype_id = vm_row["objtype_id"]
                            virtual_machine_asset_no = vm_row["asset_no"]
                            ...
                            if virtual_machine_name not in existing_virtual_machines:
                                virtual_machine_tags = getTags("object", child_entity_id)

                                netbox.virtualization.create_virtual_machine(
                                    virtual_machine_name, 
                                    cluster_name, 
                                    tags=virtual_machine_tags, 
                                    comments=virtual_machine_comment[:200] if virtual_machine_comment else "",
                                    custom_fields= {
                                        "VM_Label": virtual_machine_label[:200] if virtual_machine_label else "", 
                                        "VM_Asset_No": virtual_machine_asset_no if virtual_machine_asset_no else ""
                                    }
                                )
    ...
    if CREATE_UNMOUNTED_VMS:
        ...
        with get_db_connection() as connection:
            with get_cursor(connection) as cursor:
                ...
                vms = cursor.fetchall()
                for row in vms:
                    virtual_machine_name = row["name"]
                    virtual_machine_label = row["label"]
                    virtual_machine_comment = row["comment"]
                    virtual_machine_objtype_id = row["objtype_id"]
                    virtual_machine_asset_no = row["asset_no"]
    ...
    with get_db_connection() as connection:
        with get_cursor(connection) as cursor:
            ...
            for row in cursor.fetchall():
                PortOuterInterfaces[row["id"]] = row["oif_name"]
        ...
        with get_db_connection() as connection:
            with get_cursor(connection) as cursor:
                ...
                for row in sites:
                    site_id = row["id"]
                    site_name = row["name"]
                    site_label = row["label"]
                    site_asset_no = row["asset_no"]
                    site_comment = row["comment"]
                    ...
                    if not netbox.dcim.get_sites(name=site_name) or True:
                        ...
                        for row_id, row_name, row_label, row_asset_no, row_comment in getRowsAtSite(site_id):
                            for rack_id, rack_name, rack_label, rack_asset_no, rack_comment in getRacksAtRow(row_id):
                                ...
                                createObjectsInRackFromAtoms(atoms, rack_name, rack['id'])
    ...
    if CREATE_NON_RACKED_DEVICES:
        ...
        for objtype_id in objtype_id_names:
            ...
            children_without_parents = create_parent_child_devices(objs_list, objtype_id)
            
            # Try to recreate the children devices that didn't have parents before
            if children_without_parents:
                create_parent_child_devices(children_without_parents, objtype_id)
    ...
    if CREATE_INTERFACES:
        ...
        for device_list in (global_physical_object_ids, global_non_physical_object_ids):
            for device_name, racktables_object_id, netbox_id, objtype_id in device_list:
                ...
                with get_db_connection() as connection:
                    with get_cursor(connection) as cursor:
                        cursor.execute("SELECT id,name,iif_id,type,label FROM Port WHERE object_id=%s", (racktables_object_id,))
                ...
                for port in ports:
                    Id, interface_name, iif_if, Type, label = port["id"], port["name"], port["iif_id"], port["type"], port["label"]
    ...
    with get_db_connection() as connection:
        with get_cursor(connection) as cursor:
            ...
            for row in cursor.fetchall():
                Id, device_name = row["id"], row["name"]
                if not device_name:
                    continue
    ...
    for IP in versions:
        ...
        networks_list = list(ipv46Networks) if CREATE_IP_NETWORKS else []
